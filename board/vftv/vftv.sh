...
# Create sym link for EGL lib
ln -s "/usr/lib/libv3ddriver.so" "${ROOTFS_DIR}/usr/lib/libEGL.so"
ln -s "/usr/lib/libv3ddriver.so" "${ROOTFS_DIR}/usr/lib/libGLESv1.so"
ln -s "/usr/lib/libv3ddriver.so" "${ROOTFS_DIR}/usr/lib/libGLESv2.so"
...
# Create tar
tar -cvf "${BINARIES_DIR}/vftv.tar" -C "${ROOTFS_DIR}" .
